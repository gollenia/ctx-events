const WizardGuide = ({ state }) => {
	...
	const steps = () => {
		...
		const result = Object.keys(wizard.steps).map((step, index) => {
			if (!wizard.steps[step].enabled) {
				return null;
			}
			const classes = [
				'wizard__guide',
				!lastStep && wizard.steps[step].step == wizard?.step
					? 'wizard__guide--active text--primary'
					: false,
				wizard.steps[step].valid
					? 'wizard__guide--valid'
					: 'wizard__guide--invalid',
				wizard.steps[step].step < wizard?.step
					? 'wizard__guide--done'
					: 'wizard__guide--pending',
				lastStep &&
				wizard.steps[step].step == wizard?.step &&
				state?.response?.error
					? 'wizard__guide--error'
					: false,
				wizard.step == 3 && state?.response?.booking?.booking_id
					? 'wizard__guide--done'
					: false,
			]
				.filter(Boolean)
