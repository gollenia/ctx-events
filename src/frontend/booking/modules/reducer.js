	...
	switch (type) {
		...
		case 'REMOVE_ATTENDEE': {
			const index =
				payload.index !== undefined
					? payload.index
					: state.request.attendees.findIndex(
							(attendee) => attendee.ticket_id === payload.id,
						);
			state.request.attendees = state.request.attendees.filter(
				(_, i) => i !== index,
			);
			state.wizard.checkValidityToken = Date.now();
			return { ...state };
		...
		case 'SET_FIELD':
			if (payload.form === 'attendee') {
				state.request.attendees[payload.index].fields[payload.field] =
					payload.value;
			}
