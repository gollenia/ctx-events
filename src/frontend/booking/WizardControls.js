...
const WizardControls = ({ state, dispatch }) => {
	...
	const ATTENDEES_MISSING =
		TICKETS && request.attendees.length > 0 && !wizard.steps.tickets.valid;
	...
	const PRIVACY_MISSING =
		...
		((PAYMENT && !request.registration.data_privacy_consent) ||
			(!wizard.steps.payment.enabled &&
				REGISTRATION &&
				!request.registration.data_privacy_consent));
	...
	return (
		<div className="wizard__controls">
			{/* Back Button  */}
			{wizard.step > (wizard.steps.tickets.enabled ? 0 : 1) &&
				wizard.step < 3 && (
					<button
			...
			{wizard.step == FINAL_STEP && (
				<button
					disabled={
						!wizard.steps.registration.valid ||
						!wizard.steps.payment.valid ||
						modal.loading > 0
					}
