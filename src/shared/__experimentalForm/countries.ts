...
/**
 * ISO 3166-1 alpha-2 country codes mapped to geographic regions.
 * Regions: EU, DACH, AS, AF, OC, NA, SA
 * Duplicates from the source data have been removed.
 */
...
/**
 * Returns a sorted list of country options for the given region and locale.
 * Country names are resolved via the browser's Intl.DisplayNames API — no external request needed.
 */
export const getCountryOptions = (region: CountryRegion, locale: string): CountryOption[] => {
	...
	const codes =
		filterRegions.length === 0
			...
			: Object.entries(COUNTRY_REGIONS)
				.filter(([, regions]) => regions.some((r) => filterRegions.includes(r)))
				.map(([code]) => code);
