/**
 * Lightweight HTML sanitizer based on DOMParser + element allowlist.
...
const sanitizeNode = (node: Node, allowedTags: Set<string>): Node | null => {
	...
	if (!allowedTags.has(tagName)) {
		// Strip tag but keep text content for inline elements, discard block-level unknown tags
	...
	allowedAttrs.forEach((attr) => {
		...
		if ((attr === 'href' || attr === 'src') && !isUrlSafe(value)) return;
...
/**
 * Sanitize block-level HTML (for HtmlBlock fields and server response messages).
 * Allows headings, lists, tables, images, links, etc.
 */
...
/**
 * Sanitize inline HTML (for Checkbox labels with links or emphasis).
 * Only allows inline elements: a, strong, em, b, i, span, br, code.
 */
