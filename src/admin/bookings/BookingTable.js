...
import { Button, __experimentalHStack as HStack, Icon, __experimentalText as Text } from '@wordpress/components';
import { DataViews, filterSortAndPaginate } from '@wordpress/dataviews/wp';
import { __ } from '@wordpress/i18n';
import { drafts, notAllowed, pending, published, swatch, trash } from '@wordpress/icons';
...
const BookingTable = () => {
	...
	const fields = [
		...
		},
		{
			...
			type: 'string',
			render: ( { item } ) => {
				return formatPrice( item.price, window.eventBlocksLocalization?.currency || 'USD' );
			},
			enableSorting: false,
		},
		{
			...
			type: 'string',
			render: ( { item } ) => {
				return formatPrice( item.donation, window.eventBlocksLocalization?.currency || 'USD' );
			},
			enableSorting: false,
		},
		{
	...
	return (
		<div>
			<HStack style={ { marginBottom: '1em', padding: '12px 48px', width: 'auto' } }>
