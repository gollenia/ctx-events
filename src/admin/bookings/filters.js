...
export const makeFilter = ( field, operator, value ) => ( { field, operator, value } );
...
export function initialViewFromURL( baseView ) {
	...
	if ( q.event_id !== undefined ) {
		if ( Array.isArray( q.event_id ) ) {
			...
			if ( ids.length ) filters.push( makeFilter( FIELD_IDS.EVENT_ID, 'isAny', ids ) );
		} else {
			const id = Number( q.event_id );
			if ( Number.isFinite( id ) ) filters.push( makeFilter( FIELD_IDS.EVENT_ID, 'is', id ) );
		}
	}
...
export function viewToQuery( view ) {
	...
	for ( const f of view.filters || [] ) {
		...
		else if ( operator === 'isAny' && Array.isArray( value ) ) q[ `${ field }[]` ] = value;
	}
