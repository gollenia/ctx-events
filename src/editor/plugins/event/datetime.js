...
const datetimeSelector = () => {
	...
	if (!meta._event_start || !meta._event_end) {
		wp.data
			...
			.createNotice('warning', 'Do not forget about a date your post!', {
		...
		>
			<TextControl
				...
				onChange={(value) => {
					...
					if (
						!meta._event_rsvp_start ||
						compareTime(meta._event_rsvp_start, value)
					) {
			...
			<TextControl
				...
				onChange={(value) => {
					...
					if (
						!!meta._event_rsvp_end ||
						!compareTime(value, meta._event_rsvp_end)
					) {
						const endDate = new Date(value);
