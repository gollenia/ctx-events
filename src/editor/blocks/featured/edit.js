...
const edit = (props) => {
	...
	const categoryList = useSelect((select) => {
		...
		const categoryOptionsArray = [{ value: 0, label: '' }];
		if (!list) {
	...
	const locationList = useSelect((select) => {
		...
		const locationOptionsArray = [{ value: 0, label: '' }];
		if (!list) {
	...
	const currentEvent = useSelect(
		(select) => {
			...
			const events = getEntityRecords('postType', 'ctx-event', query);
			...
			events.forEach((event) => {
				const currentEventDate = new Date(mostRecent.meta._event_start_date);
				const nextEventDate = new Date(event.meta._event_start_date);
