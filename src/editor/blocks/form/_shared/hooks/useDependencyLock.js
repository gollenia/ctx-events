...
const useDependencyLock = (clientId, fieldName) => {
    const isSystemLocked = isSlugLocked(fieldName);

    const isReferenced = useSelect((select) => {
        ...
        return container.innerBlocks.some(block => 
    ...
    const currentLock = useSelect(select => {
        return select(blockEditorStore).getBlock(clientId)?.attributes?.lock?.remove;
    }, [clientId]);
