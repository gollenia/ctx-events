	...
	*/
		...
		if( $EM_Form->get_post() ){
			foreach($EM_Form->get_values() as $fieldid => $value){
				...
				if($fieldid == 'user_password'){
				    $booking->temporary_password = $value; //assign a random property so it's never saved
				}else{
					...
					if( !$manual_assigned_booking && (array_key_exists($fieldid, $EM_Form->user_fields) || in_array($fieldid, array('user_email','user_name'))) ){
						...
						//booking fields
						$booking->booking_meta['booking'][$fieldid] = $value;
					}
				}
			}
	...
	public static function em_booking_validate(bool $result, Booking $booking) : bool {
		...
		if( empty($EM_Form->field_values) ){
		    ...
		    $values = array();
		    if( !empty($booking->booking_meta['booking']) ){
		        $values = $booking->booking_meta['booking'];
		    }
		    if( !empty($booking->booking_meta['registration']) ){
		    	$values = array_merge($values, $booking->booking_meta['registration']);
		    }
	...
	public function em_bookings_table_rows_col($column, $booking, $format){
    	...
    	$value = $EM_Form->get_formatted_value($field, $booking->booking_meta['booking'][$column]);
