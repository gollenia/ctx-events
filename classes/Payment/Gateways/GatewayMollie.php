...
Class GatewayMollie extends Gateway implements \JsonSerializable {
	 ...
	 * @return array
	 */
	function booking_payment_feedback( $response, Booking $booking ) : array {
		...
		if (empty($description)) {
			$description = sprintf( __('%s tickets for %s', 'events'), '#_BOOKINGSPACES', '#_EVENTNAME');
		}
	...
	function define_settings_fields() {
		...
		$fields_array = array(
			...
			array(
				...
				'type'		=> 'text',
				'default' 	=> sprintf( __('%s tickets for %s', 'events'), '#_BOOKINGSPACES', '#_EVENTNAME'),
				'help' 		=> sprintf( __('Shown in the payment description in your Mollie backend. All %s are allowed.', 'events'), '<a href='. admin_url('edit.php?post_type=event&page=events-help') .' target="_blank">' . __("Events Manager Placeholders", 'events') . '</a>' ),
			),
			array(
				'id'		=> 'send_cancel_mail',
