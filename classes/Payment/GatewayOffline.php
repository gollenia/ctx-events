...
class GatewayOffline extends Gateway {
	...
	public string $title = 'Offline';
	public int $status = Booking::AWAITING_PAYMENT;
	public bool $button_enabled = true;
	
	function __construct() {
		parent::__construct();
		if( is_admin() && get_option('dbem_rsvp_enabled') ){
			add_action('em_booking_add_'.$this->slug, array(&$this, 'booking_add'), 10, 2);
	...
	function em_wp_localize_script(array $script_vars) : array {
		if( is_admin() && get_option('dbem_rsvp_enabled') ){
			$script_vars['offline_confirm'] = __('Be aware that by approving a booking awaiting payment, a full payment transaction will be registered against this booking, meaning that it will be considered as paid.','events');
		}
		return $script_vars;
	}

	function booking_form_feedback( $result, Booking $booking ) : array {
	...
	function booking_add($booking, $post_validation = false){
		...
		if( !empty($_REQUEST['payment_amount']) && !is_numeric($_REQUEST['payment_amount'])){
			$booking->errors[] = __('Invalid payment amount, please provide a number only.', 'events');
		}
	...
	/**
	 * Called instead of the filter in Gateways if a manual booking is being made
	 * @param Event $event
	 */
	function em_booking_form_footer($event){
		...
		?>
		<input type="hidden" name="gateway" value="<?php echo $this->slug; ?>" />
	...
	function gateway_settings() : void {
		...
		<tbody>
		  <?php 
		  	  Options::input( esc_html__('Success Message', 'events'), 'em_'. $this->slug . '_booking_feedback', esc_html__('The message that is shown to a user when a booking with offline payments is successful.','events') );
			  Options::input( esc_html__('IBAN', 'events'), 'em_'. $this->slug . '_iban', esc_html__('In order to generate a QR Code for payment, you have to provide a valid IBAN','events'), ["class" => 'regular-text code', 'pattern' => '[A-Z0-9]'] );
			  Options::input( esc_html__('BIC', 'events'), 'em_'. $this->slug . '_bic', esc_html__('Though not needed, some banks are only happy if you provide a BIC','events'), ["class" => 'regular-text code', 'pattern' => '[A-Z0-9]'] );			  
			  Options::input( esc_html__('Bank', 'events'), 'em_'. $this->slug . '_bank', esc_html__('Same goes with Bank name.','events') );
			  Options::input( esc_html__('Beneficiary', 'events'), 'em_'. $this->slug . '_beneficiary', esc_html__('In some countries you need to specify a beneficiary. This Data is added to the QR Code.','events') );
			  Options::input( esc_html__('Payment Deadline', 'events'), 'em_'. $this->slug . '_deadline', esc_html__('Number of days until payment has to be made','events'), ["placeholder" => "10", "type" => Options::NUMBER, "class" => 'regular-text code', 'pattern' => '[0-9]'] );
		  ?>
		</tbody>
		</table>
		<?php
	...
	/* 
	 * Run when saving  settings, saves the settings available in GatewayOffline::gateway_settings()
	 */
	function update() {
	    $gateway_options = [
			'em_'. $this->slug . '_booking_feedback',
			'em_'. $this->slug . '_iban',
			'em_'. $this->slug . '_bic',
			'em_'. $this->slug . '_bank',
			'em_'. $this->slug . '_beneficiary',
			'em_'. $this->slug . '_deadline',
		];
...
GatewayService::register_gateway('mollie', GatewayOffline::class);
