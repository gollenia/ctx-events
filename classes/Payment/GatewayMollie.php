 ...
 */
Class GatewayMollie extends Gateway {

	public string $slug = 'mollie';
	public string $title = 'Mollie';
	public int $status = Booking::AWAITING_ONLINE_PAYMENT;
	...
	function get_rest_fields() {
		return array(
			...
			"methods" => GatewayMollie::get_methods(),
		);
	}

	function get_available_status() {
		return array(
			Booking::APPROVED => __('Approved', 'events'),
			Booking::REJECTED => __('Rejected', 'events'),
			Booking::CANCELED => __('Cancelled', 'events'),
			Booking::AWAITING_ONLINE_PAYMENT => __('Waiting for Mollie', 'events'),
			Booking::PAYMENT_FAILED => __('Failed', 'events'),
		);
	}
	...
	function booking_validate($result, $booking) {
		if (isset( $_POST['paymentType'] ) && empty( $_POST['paymentType'] )) {
			$booking->errors[] =  __('Please select a payment method.', 'events');
			$result = false;
		}
	 ...
	 * @return array
	 */
	function booking_form_feedback( $response, Booking $booking ) : array {
		...
		$description = BookingView::render($booking, $description);
	...
	function update() {
		...
		$gateway_options = [
			'em_'. $this->slug . '_api_key',
			'em_'. $this->slug . '_show_methods',
			'em_'. $this->slug . '_message_free',
			'em_'. $this->slug . '_message_redirect',
			'em_'. $this->slug . '_return_page',
			'em_'. $this->slug . '_show_status',
			'em_'. $this->slug . '_status_text',
			'em_'. $this->slug . '_show_feedback',
			'em_'. $this->slug . '_description',
			'em_'. $this->slug . '_send_cancel_mail'
		];
...
GatewayService::register_gateway('mollie', GatewayMollie::class);