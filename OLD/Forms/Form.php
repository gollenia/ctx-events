...
class Form {
	...
	public static function load_block_defaults($type) {
		$defaults = [];
		$file = Plugin::get_plugin_dir() . "/src/blocks/form/" . $type . "/block.json";
			...
			switch($field['type']){
				...
				case 'checkbox':
					...
					if( $value !== "1" && !empty($field['required']) ){
						...
						$this->errors[] = $this_err .' ' . print_r($value, true) . ' ' . gettype($value);
						$result = false;
					}
					break;
				case 'radio':
				...
				case 'select':
					...
					if( (!empty($value) && !in_array($value, $values)) || (empty($value) && !empty($field['required'])) ){
						...
						$this->errors[] = ["value" => $field, "form" => $this->form_name, "field" => $field['fieldid'], "error" => $error, "attendee" => $attendee];
						$result = false;
					}		
					break;
				case 'country':
				...
				case 'email':
					if( ! is_email( $value ) ){
						$this->errors[] = __( '<strong>ERROR</strong>: The email address isn&#8217;t correct.', 'events');
						$result = false;
					}
					break;	
						...
						if ( $field['type'] == 'user_email' ) {
							if( ! is_email( $value ) ){
								...
								if( $email_exists && $email_exists != get_current_user_id() ){
									$this->errors[] = __('This email already exists in our system, please log in to register to proceed with your booking.','events');
									$result = false;	
								}
							}
						}
	...
	public static function get_form_data($form_id = 0, $associative = true) {
		...
		foreach( $blocks as $key => $block ) {
			...
			$block['attrs'] = array_merge(Form::load_block_defaults($type), $block['attrs'], ['type' => $type]);
			if($type == 'html') {
